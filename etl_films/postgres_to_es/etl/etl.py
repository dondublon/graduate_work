...
async def etl(settings: Settings, index: str) -> None:
    with closing(PostgresClient(settings.pg_dsn, cursor_factory=RealDictCursor)) as pg_conn, closing(
        RedisClient(settings.redis_dsn)
    ) as redis_conn, closing(ElasticsearchClient(settings.es_dsn)) as es_conn:
        ...
        extractor = extractor_class(
            pg_conn=pg_conn, state=state, query=QUERIES[index], state_key=state_key, extract_chunk=settings.load_chunk
        )
        loader = ESLoader(es_conn=es_conn, state=state, state_key=state_key)
